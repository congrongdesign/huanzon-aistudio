...
import { buildPdfBuffer } from "@/lib/batch-module/export";
...
export async function GET(request: NextRequest, context: { params: Promise<{ batchId: string }> }) {
  try {
    ...
    const buffer = await buildPdfBuffer(manifest);
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename*=UTF-8''${encodeURIComponent(`${manifest.batch.sourceFileName}_批量结果.pdf`)}`,
      },
    });
  } catch (error) {
